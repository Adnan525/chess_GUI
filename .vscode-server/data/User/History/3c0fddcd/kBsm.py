...
def convert_svg_to_png(svg_file, png_file):  # Tkinter not working with svg
    with open(svg_file, "rb") as f:
        svg_data = f.read()
    png_data = cairosvg.svg2png(bytestring=svg_data, output_width=300, output_height=280)
...
class ChessBoard(tk.Tk):
    def __init__(self, state: MyChess):
        ...
        self.generated_text_box = tk.Text(self, width=20, height=10, wrap=tk.WORD, pady=10, padx=10)
        self.generated_text_box.grid(row=0, column = 0, sticky="nsew")
        self.generated_text_box.insert(tk.END, "---LLM---\n")
        ...
        self.moves_text.bind("<Button-1>", self.disable_click)
        ...
        self.entry = tk.Text(user_frame, wrap="word", width=36, height=13, pady=20)
    ...
    def disable_click(self, event):
        # This function is called whenever the user clicks the text widget
        # Returning "break" from the event handler prevents default behavior
        return "break"
        
    def start_game(self):
        self.generated_text_box.insert(tk.END, "----Test Game Logic Started----\n")
